...
class Leveling(commands.Cog):
    ...
    async def handle_level_up(self, message, level, settings):
        ...
        try:
            await channel.send(
                embed=utils.success(f"{message.author.mention} awansowal na **poziom {level}**!", title="\U0001F389 Awans!")
            )
        ...
        if reward:
            ...
            if role:
                try:
                    await message.author.add_roles(role, reason=f"Nagroda za poziom {level}")
                except discord.Forbidden:
                    pass

    @app_commands.command(name="rank", description="Pokazuje Twoj poziom i XP.")
    @app_commands.describe(user="Czyj ranking (pusto = Twoj)")
    async def rank(self, interaction: discord.Interaction, user: discord.Member = None):
        ...
        e = utils.info(title=f"Ranking: {user.display_name}")
        e.set_thumbnail(url=user.display_avatar.url)
        e.add_field(name="Poziom", value=str(data["level"]))
        e.add_field(name="Pozycja", value=f"#{position}")
        e.add_field(name="XP", value=f"{data['xp']} / {needed}")
        e.add_field(name="Postep", value=f"`{bar}`", inline=False)
        await interaction.response.send_message(embed=e)

    @app_commands.command(name="leaderboard", description="Top 10 najlepszych pod wzgledem poziomu.")
    async def leaderboard(self, interaction: discord.Interaction):
        rows = await db.fetch(
            "SELECT user_id, level, xp FROM levels WHERE guild_id = $1 "
            "ORDER BY level DESC, xp DESC LIMIT 10",
            interaction.guild.id,
        )
        if not rows:
            return await interaction.response.send_message(embed=utils.info("Brak danych rankingowych."), ephemeral=True)
        ...
        for i, r in enumerate(rows):
            ...
            lines.append(f"{prefix} {name} - poziom **{r['level']}** ({r['xp']} XP)")
        e = utils.info(title=f"\U0001F3C6 Ranking - {interaction.guild.name}", description="\n".join(lines))
        await interaction.response.send_message(embed=e)

    # ---------- Konfiguracja nagrod ----------
    levels_group = app_commands.Group(name="levels", description="Konfiguracja systemu poziomow.",
                                     default_permissions=discord.Permissions(manage_guild=True))

    @levels_group.command(name="toggle", description="Wlacza/wylacza system poziomow.")
    @app_commands.describe(wartosc="Wlaczony?")
    async def toggle(self, interaction: discord.Interaction, wartosc: bool):
        ...
        await interaction.response.send_message(embed=utils.success(f"System poziomow jest teraz **{stan}**."))

    @levels_group.command(name="addreward", description="Dodaje role nadawana za osiagniecie poziomu.")
    @app_commands.describe(poziom="Poziom", rola="Rola do nadania")
    async def addreward(self, interaction: discord.Interaction, poziom: int, rola: discord.Role):
        ...
        await interaction.response.send_message(embed=utils.success(f"Za poziom **{poziom}** bedzie nadawana {rola.mention}."))

    @levels_group.command(name="delreward", description="Usuwa nagrode za poziom.")
    @app_commands.describe(poziom="Poziom")
    async def delreward(self, interaction: discord.Interaction, poziom: int):
        await db.execute(
            ...
            interaction.guild.id, poziom,
        )
        await interaction.response.send_message(embed=utils.success(f"Usunieto nagrode za poziom {poziom}."))

    @levels_group.command(name="rewards", description="Pokazuje liste nagrod za poziomy.")
    async def rewards(self, interaction: discord.Interaction):
        ...
        if not rows:
            return await interaction.response.send_message(embed=utils.info("Brak skonfigurowanych nagrod."), ephemeral=True)
        lines = [f"Poziom **{r['level']}** -> <@&{r['role_id']}>" for r in rows]
        await interaction.response.send_message(embed=utils.info(title="Nagrody za poziomy", description="\n".join(lines)))
